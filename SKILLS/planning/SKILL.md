---
name: planning
description: "Навык декомпозирования сложных задач — помогает разложить задачу на подзадачи, составить план их выполнения и выполнить его. Использует доступные навыки."
license: MIT
metadata:
  author: koda
  version: "1.0.0"
...
Навык для декомпозирования сложных задач. Помогает разложить задачу на подзадачи, составить план их выполнения и выполнить его. Использует доступные навыки.
...
1. **Изучение прежде всего** — сначала понимание проекта, потом планирование.
...
- [ ] Я провёл полный анализ задачи и контекста проекта
- [ ] Я сформулировал уточняющие вопросы, если нужно
- [ ] План разбит на логические подзадачи
- [ ] Каждая подзадача имеет чёткие критерии успеха
- [ ] План сохранён в `.koda/plans/<имя>.md`
- [ ] Я показал план пользователю и получил подтверждение
- [ ] Каждый шаг выполняется последовательно, без пропусков
- [ ] После выполнения — повторная проверка всех шагов

## Scope

- Применять ко ВСЕМ задачам, которые требуют декомпозиции
- Для простых задач (до 2 шагов) план не нужен — переходи к выполнению
- План сохраняется в `.koda/plans/<имя-плана>.md` в корне проекта
